...
$env:CRUSH_SAVE_PATH = Join-Path $env:APPDATA ("m6-build-test-"+[guid]::NewGuid()+".json")
$godotExe = Join-Path $PSScriptRoot 'godot/Godot_v4.7.2-stable_win64_console.exe'
$recordDir = Join-Path $projectRoot 'records/m6-review01'
$env:CRUSH_RECORD_DIR = $recordDir
New-Item -ItemType Directory -Force $recordDir | Out-Null
foreach ($test in @('solver_test','cardboard_test','controller_test','review02_test','aluminum_test','aluminum_controller_test','rubber_test','tire_test','progress_test','gameplay_test','cadence_test','slice_test','content_test','content_progress_test','content_visual_test','content_controller_test')) {
    $env:CRUSH_SAVE_PATH = Join-Path $env:APPDATA ("m6-test-"+[guid]::NewGuid()+".json")
    $testLog = Join-Path $recordDir "$test.log"
    $materialArgs = @('--cardboard')
    if ($test.StartsWith('aluminum')) { $materialArgs = @('--aluminum') }
    if ($test -in @('rubber_test','tire_test','progress_test','gameplay_test','slice_test','content_test','content_progress_test','content_visual_test','content_controller_test')) { $materialArgs = @() }
...
$buildDir = Join-Path $projectRoot 'builds/M6-review-01'
...
Copy-Item -LiteralPath (Join-Path $projectRoot 'README_M6.md') -Destination "$buildDir/README.md"
