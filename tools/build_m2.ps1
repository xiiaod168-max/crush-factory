...
$recordDir = Join-Path $projectRoot 'records/m2-review01'
New-Item -ItemType Directory -Force $recordDir | Out-Null
foreach ($test in @('solver_test','cardboard_test','controller_test','review02_test','aluminum_test','aluminum_controller_test')) {
    $testLog = Join-Path $recordDir "$test.log"
    if ($test.StartsWith('aluminum')) {
        & $godotExe --headless --path $projectRoot --script "tests/$test.gd" --log-file $testLog 2>&1 | Tee-Object -FilePath "$testLog.console"
    } else {
        & $godotExe --headless --path $projectRoot --script "tests/$test.gd" --log-file $testLog -- --cardboard 2>&1 | Tee-Object -FilePath "$testLog.console"
    }
...
$buildDir = Join-Path $projectRoot 'builds/M2-review-01'
...
Copy-Item -LiteralPath (Join-Path $projectRoot 'README_M2.md') -Destination "$buildDir/README.md"
