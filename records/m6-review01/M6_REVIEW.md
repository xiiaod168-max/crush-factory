# M6 Content Expansion Prototype / Review 01
...
视频 211.60 秒。实际 Windows EXE：瓶子 → 纸箱 → Lv1 铝罐 → 升级 2 → 轮胎压缩及完整回弹 → Lv2 铝罐 → 升级 3 → 木箱 → PC → 升级 4 → 微波炉 → 领取 → 退出 → 新进程重启。

使用真实界面操作信号驱动，Movie Maker 30 FPS 录制引擎画面/音轨；没有伪造奖励或加钱。完整新游戏路径从 0 到 333 金币，三次升级均靠实际回收收入。进程 18972 → 18588，重启后 333 金币 / Level 4 / 七物品正确保留。视频在真实退出与重启处拼接。

铝罐 Lv1 峰值 9.05% → Lv2 89.94%。木箱 / PC 在 Lv3 可完整处理，Lv4 进一步缩短高负载阶段；微波炉临界负载高于 Lv3 最大压力。弱压力求解测试确认新增硬物体不能达到强压力的最终压缩率。
...
- 最终 Windows EXE 完成 28 个完整循环：纸箱 5、铝罐 6、轮胎 5，四种新增物品各 3。失败 0；每次核查机器包络、回程、奖励不可重复领取及磁盘读回。
...
1280×720 / Compatibility / Intel(R) UHD Graphics。最终 EXE，无录屏、无固定帧率、无并行 GPU 或编码负载，预热 2 秒后采集 16303 帧。
平均 **59.20 FPS**，P95 **18.92 ms**，P99 **21.46 ms**，最长 **41.60 ms**。达到 30 FPS 最低目标；不声称锁定 60 FPS 或所有瞬时帧达标。录屏帧率不用于该结论。
...
版本 0.6.0-m6-review.1。EXE SHA256：`d21a4ae17ce458591cd8921c51d19dbd1045540bdadbc06f05ff0e26ae2bbac7`。
