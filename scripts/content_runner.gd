...
func run() -> void:
	...
	if resume:
		...
		check(game.progress.money==int(previous.money) and game.progress.level==4,"Wallet or upgrade lost at restart")
	...
	game._unhandled_input(debug_key)
	var order: Array[String] = ["bottle","cardboard","aluminum","tire","aluminum","crate","pc","microwave"]
	if verify:
		for i in range(2): order.append_array(["bottle","crate","pc","microwave"])
		for i in range(4): order.append_array(["cardboard","aluminum","tire"])
	for index in range(order.size()):
		if index in [3,5,7]:
			var cost: int = game.progress.upgrade_cost()
			var previous_level: int = game.progress.level
			check(game.progress.money>=cost,"Natural reward path cannot fund upgrade")
			await click(game.action_buttons.upgrades)
			await delay(0.1 if verify else 1.5)
			shot("upgrade-before-level-"+str(previous_level+1))
			var before: int = game.progress.money
			await click(game.purchase_button)
			check(game.progress.level==previous_level+1 and game.press.pressure==game.progress.max_pressure() and game.progress.money==before-cost,"Upgrade pressure or cost incorrect")
			check(not game.buy_upgrade(),"Upgrade bypassed active feedback gate")
			await delay(0.1 if verify else 2)
			shot("upgraded-level-"+str(game.progress.level))
			await click(game.hud.moment_button)
			await delay(0.1 if verify else 2)
			shot("unlocked-level-"+str(game.progress.level))
			check(game.hud.moment=="UNLOCK","Unlock moment missing")
			game.hud.dismiss_unlock()
		var id: String = order[index]
		if game.phase!="LOADED":
			var item_button: Button = game.item_buttons[id]
			item_button.get_parent().get_parent().ensure_control_visible(item_button)
			await delay(0.1 if verify else 0.6)
			await click(item_button)
		...
		while game.press.state!="COMPACTED" and not game.limit_reached and age-started<48:
			if game.peak>0.24 and not captured_peak:
				captured_peak = true
				shot(id+"-crushing")
		...
		if id=="bottle": check(row.final<row.peak-0.025 and row.final>row.peak*0.7,"Bottle recovery should be slight and permanent dent retained")
		...
		await delay(0.1 if verify else 1.5)
	if verify:
		var low: float = results[2].peak
		var high: float = results[4].peak
